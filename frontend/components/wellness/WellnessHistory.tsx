    ...
    <section className="surface-panel p-5 sm:p-6" aria-labelledby="wellness-history-title">
      ...
      <div className="mt-5 overflow-x-auto">
        <table className="w-full min-w-[58rem] text-left text-sm">
          <caption className="sr-only">Current ratings recorded in each completed Wellness Wheel snapshot. A dot means no rating was recorded.</caption>
              ...
              return (
                <tr key={assessment.id}>
                  ...
                  {WELLNESS_DIMENSIONS.map((dimension) => {
                    ...
                    const value = entry?.focus_state === "not_tracking" || entry?.current_rating === null || entry === undefined ? null : String(entry.current_rating);
                    return (
                      <td key={dimension.id} className="px-2 py-4 text-center font-mono text-xs tabular-nums">
                        <span className={active ? "font-semibold text-primary" : "text-muted-foreground"} aria-label={`${dimension.label}: ${value === null ? "not rated" : `${value} out of 10`}${active ? ", active focus" : ""}`}>
                          <span aria-hidden>{value ?? "·"}{active ? " ·" : ""}</span>
                        </span>
                      </td>
                    );
                  })}
